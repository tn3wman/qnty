...
from typing import Dict, Iterable, Final, Tuple, Optional
import inspect, re

from .dimensions_core import Dimension, dim  # your frozen/ sealed dimensions
...
def _caller_var_name(fn: str) -> str:
    ...
    if not m: raise RuntimeError("Could not auto-detect variable name")
    return m.group(1)
...
class UnitRegistry:
    ...
    def __init__(self):
        self.by_key: Dict[str, Unit] = {}
        self.preferred: Dict[Dimension, Unit] = {}
    ...
    def preferred_for(self, d: Dimension) -> Optional[Unit]:
...
class Units:
    __slots__ = ("__sealed",)
    def __init__(self): object.__setattr__(self, "_Units__sealed", False)
    def __setattr__(self, k, v):
        if self.__sealed: raise AttributeError("u is sealed; cannot modify.")
...
def add_unit(
    ...
    *,
    ...
    if name is None: name = _caller_var_name("add_unit")
...
# Add/composed units
# =======================
def attach_composed(unit: Unit, *,
                    name: str,
                    symbol: str | None = None,
                    aliases: Iterable[str] = (),
                    set_preferred: bool = False) -> Unit:
    """
...
class Quantity:
    ...
    def __mul__(self, o: "Quantity|float") -> "Quantity":
        if isinstance(o, Quantity): return Quantity(self.value*o.value, self.dim*o.dim)
        return Quantity(self.value*float(o), self.dim)
    def __truediv__(self, o: "Quantity|float") -> "Quantity":
        if isinstance(o, Quantity): return Quantity(self.value/o.value, self.dim/o.dim)
        return Quantity(self.value/float(o), self.dim)
    def __pow__(self, k: int) -> "Quantity": return Quantity(self.value**k, self.dim**k)
    def __add__(self, o: "Quantity") -> "Quantity":
        if self.dim != o.dim: raise TypeError("Dimension mismatch in +")
        return Quantity(self.value+o.value, self.dim)
    def __sub__(self, o: "Quantity") -> "Quantity":
        if self.dim != o.dim: raise TypeError("Dimension mismatch in -")
        return Quantity(self.value-o.value, self.dim)
    def to(self, unit: Unit) -> float:
        if unit.dim != self.dim: raise TypeError("Conversion dimension mismatch")
        return (self.value - unit.si_offset)/unit.si_factor
    def __repr__(self) -> str:
        pref = ureg.preferred_for(self.dim)
        return f"{self.to(pref):.6g} {pref.symbol}" if pref else f"{self.value:.6g} [Dim={self.dim}]"
...
def write_units_stub(path: str = "units.pyi") -> None:
    with open(path, "w", encoding="utf-8") as f:
        ...
        f.write("from .units_core import Unit\n\n")
        f.write("class Units:\n")
...
def seal_units() -> None:
    ureg.seal()
    object.__setattr__(u, "_Units__sealed", True)
