...
class ProblemMeta(type):
    ...
    def __new__(mcs, name: str, bases: Tuple[type, ...], namespace: ProxiedNamespace, **kwds: Any) -> type:
        ...
        try:
            ...
            setattr(cls, '_original_sub_problems', sub_problem_proxies)
            ...
            setattr(cls, '_proxy_configurations', proxy_configurations)
            
            # Collect Check objects from class attributes
            checks = {}
            for attr_name, attr_value in namespace.items():
                if isinstance(attr_value, Check):
                    checks[attr_name] = attr_value
            
            setattr(cls, '_class_checks', checks)
...
class ProxiedNamespace(dict):
    ...
    def _is_variable_with_auto_symbol(self, value: Any) -> bool:
        ...
        try:
            from qnty.variable import TypeSafeVariable as Variable
