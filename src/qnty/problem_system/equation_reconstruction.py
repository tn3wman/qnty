...
from qnty.expression import (
    VariableReference, BinaryOperation, Constant, 
    sin, cos, min_expr, max_expr, ConditionalExpression,
    UnaryFunction
)
# Create aliases for backward compatibility with old optinova code
UnaryOperation = UnaryFunction  # qnty uses UnaryFunction instead
BinaryFunction = BinaryOperation  # Use BinaryOperation as closest equivalent


# Type aliases for better readability
VariableDict = Dict[str, Variable]
NamespaceMapping = Dict[str, str]
ReconstructionResult = Optional[Equation]
...
ValidExpressionType = Union[
    VariableReference, BinaryOperation, UnaryOperation, Constant, BinaryFunction,
...
class EquationReconstructor:
    ...
    def __init__(self, problem: Any) -> None:
        ...
        self._all_variable_names: Optional[Set[str]] = None
            ...
            for missing_var in missing_vars:
                if missing_var in eq_str:
                    ...
                    if reconstructed_expr:
                        ...
                        if (hasattr(equation.lhs, 'symbol') and 
                            hasattr(equation.lhs, 'symbol') and 
                            isinstance(getattr(equation.lhs, 'symbol', None), str)):
                            symbol = getattr(equation.lhs, 'symbol')
    ...
    def _reconstruct_expression_from_mapping(self, composite_expr: str, namespace_mappings: NamespaceMapping) -> Optional[Any]:
        """
    ...
    def _get_lhs_variable(self, equation: Equation) -> Optional[Variable]:
        """
        ...
        """
        if not hasattr(equation.lhs, 'symbol'):
            return None
            
        symbol = getattr(equation.lhs, 'symbol', None)
        if not isinstance(symbol, str) or symbol not in self.variables:
            return None
    ...
    def _reconstruct_malformed_proxy_expression(self, equation: Equation, malformed_vars: List[str]) -> Optional[Any]:  # noqa: ARG002
        """
    ...
    def _rebuild_expression_from_malformed(self, expr: Any) -> Optional[Any]:
        """
        ...
        if isinstance(expr, VariableReference):
            ...
            if ' = ' in var_symbol:
                ...
                # This is a composite expression pattern - check if the Variable has a proper _expr first
                if hasattr(expr, 'variable') and hasattr(expr.variable, '_expr'):
                    print(f"DEBUG: Using existing _expr for composite variable: {var_symbol}")
                    print(f"DEBUG: Variable value: {expr.variable.quantity}")
                    print(f"DEBUG: _expr: {expr.variable._expr}")
                    self.logger.debug(f"Using existing _expr for composite variable: {var_symbol}")
                    return expr.variable._expr  # Use the existing expression tree directly
                else:
                    print(f"DEBUG: No _expr found for {var_symbol}, using pattern reconstruction")
                    # Fallback to pattern reconstruction
                    return self.parse_composite_expression_pattern(var_symbol)
            else:
                return None
                
        elif hasattr(expr, 'symbol') and isinstance(expr.symbol, str):
            ...
            # Handle functions like min_expr, max_expr, sin, etc.
            left_rebuilt = self._rebuild_expression_from_malformed(expr.left)
            right_rebuilt = self._rebuild_expression_from_malformed(expr.right)
            
            if left_rebuilt and right_rebuilt:
                if expr.function_name == 'min':
                    return min_expr(left_rebuilt, right_rebuilt)
                elif expr.function_name == 'max':
                    return max_expr(left_rebuilt, right_rebuilt)
                else:
                    return BinaryFunction(expr.function_name, left_rebuilt, right_rebuilt)
                    
        elif isinstance(expr, UnaryOperation):
            operand_rebuilt = self._rebuild_expression_from_malformed(expr.operand)
            if operand_rebuilt:
                if expr.operator == 'sin':
                    return sin(operand_rebuilt)
                elif expr.operator == 'cos':
                    return cos(operand_rebuilt)
                else:
                    return UnaryOperation(expr.operator, operand_rebuilt)
    ...
    def parse_composite_expression_pattern(self, composite_symbol: str) -> Optional[Any]:
        """
    ...
    def _handle_simple_composite_patterns(self, pattern: str) -> Optional[Any]:
        """
        ...
        # Handle patterns like "S_r / header_S"
            ...
            if len(parts) == 2:
                ...
                if left_part in self.variables and right_part in self.variables:
                    from ..primitives.expression import VariableReference
                    left_var_ref = VariableReference(self.variables[left_part])
                    right_var_ref = VariableReference(self.variables[right_part])
                    return left_var_ref / right_var_ref
    ...
    def parse_malformed_variable_pattern(self, malformed_symbol: str) -> Optional[Any]:
        """
    ...
    def _rebuild_mathematical_pattern(self, pattern: str, existing_vars: List[str]) -> Optional[Any]:
        """
    ...
    def parse_and_rebuild_expression(self, expr: Any, missing_vars: List[str]) -> Optional[Any]:
        """
        ...
        if isinstance(expr, VariableReference):
            if expr.symbol in missing_vars:
                # This is a composite expression - try to parse and rebuild it
                return self.parse_composite_expression_pattern(expr.symbol)
            return expr
