...
class SimultaneousEquationSolver(BaseSolver):
    ...
    def solve(
        ...
        working_variables = {symbol: variable for symbol, variable in variables.items()}
    ...
    def _solve_matrix_system(self, equations: List[Equation], unknown_variables: List[str], 
                           working_variables: Dict[str, Variable]) -> Optional[np.ndarray]:
        """
        ...
        if condition_number > self.MAX_CONDITION_NUMBER:
            if self.logger:
                ...
                if np.isinf(condition_number):
                    self.logger.debug(f"Matrix is singular (cond=inf), falling back to iterative solver")
                else:
    ...
    def _extract_matrix_system(self, equations: List[Equation], unknown_variables: List[str],
                             variables: Dict[str, Variable]) -> Tuple[Optional[np.ndarray], Optional[np.ndarray]]:
        """
    ...
    def _extract_linear_coefficients_vector(self, equation: Equation, unknown_variables: List[str],
                                          variables: Dict[str, Variable]) -> Optional[List[float]]:
        """
        ...
        try:
            ...
            residual_test_cases = np.zeros(num_unknowns) if num_unknowns > 10 else []
            ...
            for variable_index in range(num_unknowns):
                ...
                for unknown_index, variable_name in enumerate(unknown_variables):
                    test_value = 1.0 if unknown_index == variable_index else 0.0
                    test_variables[variable_name] = Variable("test", variable_name, 
                                                 Qty(test_value, test_variables[variable_name].quantity.unit),
                                                 is_known=True)
                
                residual = self._calculate_equation_residual(equation, test_variables)
            ...
            for variable_name in unknown_variables:
                test_variables[variable_name] = Variable("test", variable_name, 
                                             Qty(0.0, test_variables[variable_name].quantity.unit),
                                             is_known=True)
            baseline_residual = self._calculate_equation_residual(equation, test_variables)
    ...
    def _update_variable_with_solution(self, variable_symbol: str, solution_value: float, 
        ...
        solved_variable = Variable(original_name, variable_symbol, solution_quantity, is_known=True)
