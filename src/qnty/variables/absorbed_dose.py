...
class AbsorbedDoseSetter(TypeSafeSetter):
    """Absorbed dose-specific setter with only absorbed dose units."""
    
    def __init__(self, variable: 'AbsorbedDose', value: float):
        super().__init__(variable, value)
    
    # Only absorbed dose units available - compile-time safe!
    @property
    ...
    def gram_rads(self) -> 'AbsorbedDose':
        self.variable.quantity = FastQuantity(self.value, AbsorbedDoseUnits.gram_rad)
        return cast('AbsorbedDose', self.variable)
...
class AbsorbedDose(TypedVariable):
    """Type-safe absorbed dose variable with expression capabilities."""
...
class AbsorbedDoseModule(VariableModule):
    """Absorbed dose variable module definition."""
