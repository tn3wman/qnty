...
class DimensionGenerator:
    ...
    def generate_common_signatures_dict(self) -> list[str]:
        ...
        sorted_sigs = sorted(self.common_signatures.items(), key=lambda x: (sum(abs(v) for v in x[0]), x[0]))
        ...
        for dim_tuple, signature in sorted_sigs[:max_cache_entries]:
            ...
            matching_dims = [name for name, dims in self.all_dimensions.items() if self.tuple_from_dims(dims) == dim_tuple]
