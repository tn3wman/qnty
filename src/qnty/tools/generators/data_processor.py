                ...
                for prefix_enum in prefixes:
                    ...
                    if prefixed_name not in existing_units:
                        ...
                        prefixed_unit = {
                            ...
                            "aliases": [prefix.apply_to_symbol(unit_data.get("notation", ""))] if unit_data.get("notation") else [],
...
def calculate_statistics(unit_data: dict[str, Any]) -> dict[str, Any]:
    ...
    generated_units = sum(sum(1 for unit in field_data.get("units", []) if unit.get("generated_from_prefix", False)) for field_data in unit_data.values())

    return {"total_units": total_units, "total_fields": total_fields, "generated_prefixed_units": generated_units, "original_units": total_units - generated_units}
    ...
    for field_name, field_data in parsed_data.items():
        for unit_data in field_data["units"]:
            ...
            if unit_name in PREFIXABLE_UNITS:
                ...
                base_units[unit_name].append({"unit_data": unit_data, "field_name": field_name, "prefixes": PREFIXABLE_UNITS[unit_name]})
