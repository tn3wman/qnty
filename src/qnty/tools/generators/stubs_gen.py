...
def generate_quantities_pyi(parsed_data: dict, dimension_mapping: dict) -> str:
    ...
    lines.extend(["# ===== QUANTITY CLASSES =====", "# Type stubs for quantity classes with setter relationships", ""])
...
def main():
    ...
    fields_by_units = [(field_name, len(field_data.get("units", [])), field_data.get("field", "")) for field_name, field_data in augmented_data.items() if field_data.get("units")]
