...
def generate_setters_pyi(parsed_data: dict, dimension_mapping: dict) -> str:
    """Generate the field_setter.pyi stub content."""
    ...
    lines = [
        ...
        f"Contains {len(fields_with_units)} setter types with {sum(len(f.get('units', [])) for f in parsed_data.values())} total unit properties.",
        "",
    ...
    lines.append("# Type stubs for setter classes with unit properties")
    lines.append("")
    ...
    for field_name, field_data in fields_with_units:
        ...
        lines.append(f'    """{class_name}-specific setter with unit properties."""')
        lines.append("")
        lines.append("    __slots__ = ()")
        lines.append("")

        # Generate property stubs for each unit, avoiding duplicates
        ...
        for unit_data in field_data["units"]:
            ...
            lines.append(f"    def {property_name}(self) -> FieldQnty: ...")
            lines.append("")
            ...
            for alias in aliases:
                ...
                if alias != property_name and alias.isidentifier() and alias not in generated_properties:
                    ...
                    lines.append("    @property")
                    lines.append(f"    def {alias}(self) -> FieldQnty: ...")
                    lines.append("")
...
def generate_setters(parsed_data: dict, dimension_mapping: dict) -> str:
    ...
    for field_name, field_data in fields_with_units:
        ...
        for unit_data in field_data["units"]:
            ...
            lines.append(f"        unit_const: Final = field_units.{units_class_name}.{property_name}")
...
def main():
    ...
    base_path = Path(__file__).parents[2]  # Go up to qnty root
    ...
    py_output_file = generated_path / "field_setter.py"
    pyi_output_file = generated_path / "field_setter.pyi"
    ...
    print("Generating field_setter.py...")
    py_content = generate_setters(augmented_data, dimension_mapping)

    # Generate stub file
    print("Generating field_setter.pyi...")
    pyi_content = generate_setters_pyi(augmented_data, dimension_mapping)

    # Write output files
    save_text_file(py_content, py_output_file)
    save_text_file(pyi_content, pyi_output_file)
    print(f"Generated setters file: {py_output_file}")
    print(f"Generated stub file: {pyi_output_file}")
