...
def generate_quantities_pyi(parsed_data: dict, dimension_mapping: dict) -> str:
    """Generate the field_vars.pyi stub content."""
    ...
    lines = [
        ...
        f"Contains {len(fields_with_units)} quantity types with {sum(len(f.get('units', [])) for f in parsed_data.values())} total units.",
        "",
    ...
    lines.append("# Type stubs for quantity classes with setter relationships")
    lines.append("")
    ...
    for field_name, field_data in fields_with_units:
        ...
        lines.append(f"class {class_name}(FieldQnty):")
        ...
        # Generate value and unit property stubs
        lines.append("    ")
        lines.extend(generate_value_unit_properties(display_name, stub_only=True))
...
def main():
    ...
    py_output_file = generated_path / "field_vars.py"
    pyi_output_file = generated_path / "field_vars.pyi"
    ...
    print(f"Generated quantities file: {py_output_file}")
