...
class TestDivisionByOne:
    ...
    def test_different_dimension_division_by_one(self):
        ...
        # Expected: 10 square_meter / 1 meter = 10000 millimeter (unit system chooses mm)
        ...
        # Expected: 25 cubic_meter / 1 square_meter = 25000 millimeter (unit system chooses mm)
        ...
        # Expected: 100 pascal / 1 meter = 100 pascal/meter
        assert result.value == pytest.approx(100.0)
        # Should have dimension different from both pressure and length
        pressure_dim = -1 if pressure.quantity is None else pressure.quantity._dimension_sig
        length_dim = 2 if length2.quantity is None else length2.quantity._dimension_sig
        assert result._dimension_sig != pressure_dim
    ...
    def test_multiple_variable_types_with_one(self):
        ...
        for numerator, denominator, expected_value in test_cases:
            ...
            assert result.value == pytest.approx(expected_value), f"Failed for {type(numerator).__name__} / {type(denominator).__name__}"
            assert result._dimension_sig == 1, f"Expected dimensionless result for {type(numerator).__name__} / {type(denominator).__name__}"
    ...
    def test_division_by_negative_one(self):
        ...
        assert result.value == pytest.approx(-100.0)
        # Force dimension preserved (Force has dimension sig typically around 5)
        force_dim = 5 if force.quantity is None else force.quantity._dimension_sig
        assert result._dimension_sig == force_dim
    ...
    def test_chain_divisions_with_ones(self):
        ...
        # Intermediate result should be 20 m (Length)
        intermediate = first_div.evaluate(context)
        # Expected: 20 square_meter / 1 meter = 20000 millimeter (unit system chooses mm)
        assert intermediate.value == pytest.approx(20000.0)
