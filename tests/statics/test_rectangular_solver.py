...
from qnty.problems.statics.rectangular_solver import solve_class
...
def test_rectangular_problem(problem_class):
    ...
    for comp in problem.result.components:
        ...
        # Get actual vector from problem
        actual_vec = None
        for attr_name in dir(problem_class):
            if attr_name.startswith('_'):
                continue
            attr = getattr(problem_class, attr_name)
            if isinstance(attr, Vector) and (attr.name == vec_name or attr_name == vec_name):
                actual_vec = attr
                break
