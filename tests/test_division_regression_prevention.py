...
class TestDivisionOptimizationRegression:
    ...
    def test_division_by_one_with_same_dimensions_returns_dimensionless(self):
        ...
        for num_val, num_unit, den_val, den_unit, var_type in test_cases:
            ...
            assert result.value == pytest.approx(expected_value), \
                f"Division result incorrect for {var_type.__name__}: expected {expected_value}, got {result.value}"
            
            assert result._dimension_sig == 1, \
                f"Division of same dimensions must be dimensionless for {var_type.__name__}: got dimension {result._dimension_sig}"
            
            assert result.unit.symbol == "", \
                f"Dimensionless result must have empty unit symbol for {var_type.__name__}: got '{result.unit.symbol}'"
    ...
    def test_mixed_dimension_divisions_with_one_values(self):
        ...
        for num_type, num_val, num_unit, den_type, den_val, den_unit in test_cases:
            ...
            assert result._dimension_sig != numerator.quantity._dimension_sig, \
                f"Result should have different dimension than numerator: {num_type.__name__}/{den_type.__name__}"
