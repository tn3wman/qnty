...
from qnty.core.dimension_catalog import (
    ACCELERATION,
    AREA,
    DIMENSIONLESS,
    FORCE,
...
class TestDimensionSignature:
    ...
    @pytest.mark.parametrize(
        "dim_name,exponent,expected_prime",
        [
            ("length", 1, 2),
            ("mass", 1, 3),
            ("time", 1, 5),
    ...
    def test_create_compound_dimensions(self, length, mass, time, expected):
        ...
        if length >= 0 and mass >= 0 and time >= 0:
            expected_sig = BaseDimension.LENGTH**length * BaseDimension.MASS**mass * BaseDimension.TIME**time
    ...
    def test_create_with_all_dimensions(self):
        ...
        dim = DimensionSignature.create(length=1, mass=1, time=1, current=1, temp=1, amount=1, luminosity=1)
        expected = BaseDimension.LENGTH * BaseDimension.MASS * BaseDimension.TIME * BaseDimension.CURRENT * BaseDimension.TEMPERATURE * BaseDimension.AMOUNT * BaseDimension.LUMINOSITY
        assert dim._signature == expected
