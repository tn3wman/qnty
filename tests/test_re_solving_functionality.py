...
def test_original_variable_states_preserved():
    ...
    assert problem.P.is_known == True
    assert problem.D.is_known == True
    assert problem.factor.is_known == True
    assert problem.result.is_known == False
    
    # Solve
    problem.solve()
    
    # After solving, result should become known
    assert problem.result.is_known == True
